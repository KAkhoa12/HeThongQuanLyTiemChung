...
import { FaCalendarAlt, FaClock, FaUser, FaCheck, FaTimes, FaEye, FaSearch, FaFileAlt, FaPhone, FaEnvelope, FaStethoscope, FaUsers } from 'react-icons/fa';
...
const DoctorAppointmentManagementPage: React.FC = () => {
  ...
  const [viewMode, setViewMode] = useState<'doctors' | 'appointments'>('doctors');
  ...
  useEffect(() => {
    if (selectedDoctor && viewMode === 'appointments') {
      fetchDoctorAppointments(selectedDoctor);
    }
  }, [selectedDoctor, viewMode, fetchDoctorAppointments]);
  ...
  const viewAppointmentDetails = (appointmentId: string) => {
    ...
    // Reset approval states when opening details modal
    setApprovalStatus('');
    setRejectionReason('');
  };
  ...
  return (
    <>
      <div className="mx-auto max-w-7xl p-4 md:p-6 2xl:p-10">
        <div className="mb-6">
          <Breadcrumb pageName="Quản lý lịch hẹn bác sĩ" />
        </div>

        {/* Header */}
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-2">Quản Lý Lịch Hẹn Bác Sĩ</h1>
          <p className="text-gray-600">Xem và quản lý lịch hẹn của tất cả bác sĩ</p>
        </div>
            ...
            {doctorsLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
                {doctors.map((doctor) => (
                  <div
                    key={doctor.id}
                    className="bg-gray-50 rounded-lg p-6 hover:shadow-md transition-shadow cursor-pointer border-2 border-transparent hover:border-blue-200"
        ...
        {/* Appointments View */}
        {viewMode === 'appointments' && selectedDoctor && (
          <>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{formatDate(appointment.ngayDangKy)}</p>
                                     ...
                                     {/* Approval Section - Only show if status is pending */}
                   {appointment.trangThai === 'Pending' && (
                    <>
                ...
                <div className="flex justify-end space-x-3 mt-6">
                  ...
                  >
                    Đóng
                                     ...
                                     {/* Show approval button only if status is pending and decision is selected */}
                   {appointment.trangThai === 'Pending' && approvalStatus && (
                    <button
                      onClick={() => handleApproval(appointment.maPhieuDangKy, approvalStatus, rejectionReason)}
                                             disabled={(approvalStatus === 'Từ chối' && !rejectionReason) || approving}
                      className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed"
                    >
                      {approving ? 'Đang xử lý...' : 'Xác nhận'}
                    </button>
