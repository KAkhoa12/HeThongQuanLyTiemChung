...
import { getAllServices, deleteService, getAllServiceTypesNoPage } from '../../services/service.service';
...
const ServiceListPage = () => {
  ...
  const fetchServiceList = async () => {
    try {
      setLoading(true);
      const response: PagedResponse<Service> = await getAllServices(
        pagination.currentPage,
        pagination.pageSize
      );
  ...
  const handleFilterByType = async (typeId: string) => {
    setSelectedType(typeId);
    try {
      setLoading(true);
      // If no type is selected, get all services
      if (!typeId) {
        fetchServiceList();
        return;
      }
      ...
      // In a real app, you'd call a specific API endpoint for filtering
      const filteredServices = response.data.filter(
        (service) => service.serviceTypeId === typeId
      );
      
      setServiceList(filteredServices);
    } catch (error) {
      console.error('Filter failed:', error);
      toast.error('Lọc dịch vụ thất bại');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6">
          ...
          <div className="flex flex-col sm:flex-row gap-4">
            <select
              className="rounded-md border border-stroke bg-transparent py-2 pl-4 pr-6 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
              value={selectedType}
